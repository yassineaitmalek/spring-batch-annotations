...
public class DateUtility {
  ...
  public static String nameWithDate(String name) {
    return name + "_" + nowDateTimeFormatted();
  }
  ...
  public static LocalDate convertDateToLocalDate(Date date) {
    ...
    return Optional.ofNullable(date)
        .map(e -> LocalDateTime.ofInstant(e.toInstant(), ZONE_AFRICA_CASABLANCA).toLocalDate())
        .orElse(null);
  }
  ...
  public static Date convertLocalDateToDate(LocalDate localDate) {
    ...
    return Optional.ofNullable(localDate)
        .map(e -> Date.from(e.atStartOfDay(ZONE_AFRICA_CASABLANCA).toInstant()))
        .orElse(null);
  }
  ...
  public static long calculateMinutesDifference(
  ...
  public static List<LocalDate> getDatesBetween(LocalDate startDate, LocalDate endDate) {
    ...
    if (startDate.isAfter(endDate)) {
      throw new IllegalArgumentException("start date can not be greater than end date");
    }
    return LongStream.rangeClosed(0, endDate.toEpochDay() - startDate.toEpochDay())
